...
async function fetchApi(endpoint, options = {}) {
  ...
  if (!res.ok) {
    ...
    const errorMessage = error.details
      ? `${error.error || error.message || "API error"}: ${error.details}`
...
export const api = {
  ...
  seedCategories: () => fetchApi("/categories/seed", { method: "POST" }),
  // Orders (Exercise 3, 5)
  getOrders: (params = {}) => {
    const query = new URLSearchParams(params).toString();
  ...
  generateDescription: (productId) =>
    fetchApi("/ai/describe", {
      method: "POST",
      body: JSON.stringify({ product_id: productId }),
    }),
  getStockAdvice: () => fetchApi("/ai/stock-advice", { method: "POST" }),
  summarizeOrders: () => fetchApi("/ai/summarize-orders", { method: "POST" }),
